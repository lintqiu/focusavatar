...
通过技能入口启动后，会提示获取 accessKeyId / accessKeySecret，然后选择操作：

- **[1] 提交任务（生成视频）**：依次输入 MP3、MP4、文字 → 确认 → 提交；支持轮询直至完成并输出视频链接。
- **[2] 查询任务结果**：输入任务单号 `orderNo`，调用查询接口获取状态与视频链接。

仅查询结果时，选择「查询任务结果」并输入 orderNo 即可。
